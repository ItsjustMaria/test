...
def find_archive_folders(source_root, archive_numbers, logger):
    ...
    logger.info(f'Searched in keep? What does this var tell us? {keep}')
    ...
    for root in archive_roots:
        for dirpath, _dirnames, bestandsnamen in os.walk(root):
            for file in bestandsnamen:
                full = os.path.join(dirpath, file.lower())
...
def resolve_file(bestandsnaam, unique_paths):
    ...
    file = bestandsnaam
    #print(key)
    if file in unique_paths:
        candidates = file
        print(candidates['d10100000174.jpg'])
        by_name[file.lower()].append(full)               
...
def process(df, source_root, dest_root, mode, dry_run, logger):
    ...
    for i, toegangsnummer in enumerate(needed_archives, 1):
        ...
        unique_paths = index_files_for_archive(archive_folders[toegangsnummer], logger)
        #print(file_indexes[toegangsnummer])
    ...
    for n, row in enumerate(df.itertuples(index=False), 1):
        ...
        candidates, match_type = resolve_file(bestandsnaam, unique_paths)
        #print(f'the files by_name: \n {by_name}\n')
        if match_type == "none":
            errors.append({
                "toegangsnummer": toegangsnummer, "uuid": uuid, "bestandsnaam": bestandsnaam,
