...
kwrgs = {
        ...
        'test_amount': 5,
        'down_amount' : 100,
        'test_concepts' : 'data/test_records.ttl',
        ...
        'record_uuids' : "data/record_uuids_with_adam.csv",            #### !!!! Location of deed turtle
        'records' : 'data/records_with_adam.ttl',      
...
def retrieve_concept_turtle_from_memorix(**kwrgs):
    ...
    print('Kom ik wel in de functie?')
    '''# Concept vocabulaire turtle uit memorix halen
    response = api.list_concepts( kwrgs['vocabulair'])
    tqdm(response.text,  file=open(kwrgs['concept_turtle'], 'w', encoding='utf-8'))
    ...
    kwrgs['concept_turtle'] = response.text

    g = Graph()
    g.parse(kwrgs['concept_turtle'], format="turtle")

    #get data from turtle
    pattern = r'/vocabularies/concepts/([^/>]+)'
    uuid = pattern.group(1) 
    concept_uri = URIRef(f"{PREFIX}/resources/vocabularies/concepts/{uuid}")
    concept_uris = set()
    ...
    '''# Zet limiet in kwrgs dict
    limited_triples = list(g)[:kwrgs['down_amount']]

    # Test graph voor toepassing limieten per triple <-- <-- WERKT DUS NIET ALS JE EEN RECORD MET MEERDERE TRIPLES HEBT
    # DAN MOET JE SPLITSEN OP RECORD : FOR RECORD IN G.SUBJECTS(RDFLIB.RDF.TYPE, MEMORIX.RECORD):
    limited_g = Graph()
    for concept_uri in limited_triples:
        limited_g.add(concept_uri)

    # Testoutput naar file. MAG BLIJVEN STAAN IN PRODUCTIE. TEST_AMOUNT WORDT IN DE KWRGS BEPAALD. 
    limited_g.serialize(destination=kwrgs['test_concepts'], format="turtle")

    # Turtle Concepts lezen
    for s in g.subjects(rdflib.RDF.type, SKOS.Concept):
        s_str = str(s)

    # Match maken met concept UUID
        ...
        for uri in concept_uris:
            if s == uri:
                if uri not in records:
                    records[uri] = []
                records[uri].append((s, p, o))

    # Limit to N records
    #N = 100  # or kwargs['down_amount']
    kwrgs['concept_turtle'] = list(records.items())[:kwrgs['down_amount']]

    print(kwrgs['concept_turtle'])

    # Concept predicaten naar een lijst voor verwerking elders
    ...
    # Create a new graph with only the first N records
    kwrgs['concept_turtle'] = Graph()
    for uri, triples in kwrgs['concept_list']:
        for triple in triples:
            kwrgs['concept_list'].add(triple)

            # Totalen vaststellen om te bepalen of je data verliest
            kwrgs['total_concept_uuids'].append(uuid)   

    # Save the limited graph
    kwrgs['concept_turtle'].serialize(destination=kwrgs['concept_turtle'], format="turtle")

    print(f'This is the turtle amount: \n {len(kwrgs['total_concept_uuids'])}\n\n' +
          f'This is the concept list: \n {kwrgs['concept_list']}\n\n')
    
        
    return kwrgs['concept_list'], kwrgs['total_concept_uuids'], response
...
def retrieve_uuid_from_memorix(**kwrgs):
    ...
    import get_uuid_with_adam  
    # Get uuids with query 
    response = get_uuid_with_adam.main(env, kwrgs['record_uuids'])
    #response = 200  # TEMP
    ...
    return kwrgs['df_record_uuids'], response
...
def get_turtle_for_record_with_uuid(**kwrgs):
    ...
    return kwrgs['predicates'], kwrgs['total_predicates'], response
...
def match_data(
    ...
    print(f'Deze print ie niet {kwrgs['predicates_df']}')
...
def main(**kwrgs):
    ...
    try:
        ...
        kwrgs['concept_list'], kwrgs['total_concept_uuids'], response = retrieve_concept_turtle_from_memorix(**kwrgs) 

        print('After the function')
        input('I don\'t want it to go on')
        ...
        if response.status_code == 200:  #response.status_code == 200:          
            ...
            log.error(f"FAIL {row.uuid}")
            log.error(response) #log.error(response.text)
    ...
    try:
        ...
        kwrgs['df_record_uuids'], response = retrieve_uuid_from_memorix(**kwrgs)

        if response== 200:  #response.status_code == 200:
    ...
    try:   
        ...
        kwrgs['predicates'], kwrgs['total_predicates'], response = get_turtle_for_record_with_uuid( **kwrgs )
        
        print(kwrgs['predicates'])
        if response== 200:  #response.status_code == 200:
    ...
    try:
        ...
        kwrgs['predicates_df'], kwrgs['outliers_df'] = match_data(**kwrgs)
