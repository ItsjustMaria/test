...
DEED = Namespace(f"https://{PREFIX}.memorix.io/resources/recordtypes/Deed#")
...
# Step 1 function
def retrieve_deed_turtle_from_memorix(api, turtle_description, deed_turtle):
    
    # Get turtle for record to update 
    try:
        response = api.get_record_type( turtle_description)
        print(response.text,  file=open(deed_turtle, 'w', encoding='utf-8'))
        return response
    except:
        log.error(f'Could not get record_type {turtle_description} from api {api} with response {response}')
        errors.append({'fn: retrieve_deed_turtle_from_memorix': [turtle_description, deed_turtle, response]})
...
# Step 3 function
def retrieve_uuid_from_memorix(env, street_uuids):
    ...
    response = get_uuid.main(env, street_uuids)

    df = pd.read_csv(street_uuids,
                                  
        sep=";",             
        dtype={ "uuid": str
           })
    
    df_street_uuids = pd.DataFrame(df)

    return df_street_uuids

# Step 4 function
def turtle_to_list( concept_turtle):
...
def read_data_with_panda(data):
    ...
    df_alt_streets = pd.DataFrame(df)
    
    return df_alt_streets

# Step 6 function
def process_data(api, df_street_uuids):

    #print(f'I get in the MOTHERFOCKING FUNCTION with {df_street_uuids}')
    # loop through data with progress meter
    #for index, row in tqdm(data.iterrows(), total=data.shape[0]):
    first = df_street_uuids.iloc[0,0]
    try:
        response = api.get_record(first)
        print(response.text, file=open('turtle', 'w', encoding='utf-8')) !!!!!!!!!!  #####!!!!!!!!  'turtle' moet een variabele hebben
        if response.status_code != 200:
            log.info("...Try to read again...")
            time.sleep(3)
            response = api.get_record(first)
            if response.status_code != 200:
                log.error(f"Reading failed for {first}")

        return response
                
    except:
        log.error(f"FAILED TRANSFORMATION {first}")
    
    #for index, row in df_street_uuids.iterrows():
    #    print(type(row))
    #    print(row.shape)
    #    print(row.head())
    #    break
...
# Step 7 function
def process_some_other_data(api, data):

    # loop through data with progress meter
    for index, row in tqdm(data.iterrows(), total=data.shape[0]):
        log.info(f"START {row.auuid}")
        try:
...
def cli(ctx, env):
    ...
    ctx.obj["street_uuids"] = "../data/street_uuids.csv"            #### !!!! Location of deed turtle
    ctx.obj["excel_sheet"] = "../data/concept_streets.xlsx"         #### !!!! Location of concepts
...
def download_record_uuid(ctx):
    ...
    retrieve_uuid_from_memorix( ctx.obj["env"], ctx.obj['street_uuids'])

# Step 4 cli command
@cli.command()
def concept_turtle_to_list(ctx):
    
    turtle_to_list( ctx.obj["concept_turtle"])

# Step 5 cli command
@cli.command()
def read_added_datasheet(ctx):
    
    read_data_with_panda( ctx.obj["data"])

# Step 6 cli command
@cli.command()
def concept_turtle_to_excel(ctx):
    
    process_data(ctx.obj['api'], ctx.obj['df_street_uuids'])
...
def pipeline(ctx, data):
    ...
    #deed_turtle = retrieve_deed_turtle_from_memorix(ctx.obj["api"], ctx.obj['Deed'])
    #ctx.obj['deed_turtle'] = deed_turtle
    ...
    #concept_turtle = retrieve_concept_turtle_from_memorix(ctx.obj["api"], ctx.obj['vocabulair']) 
    ...
    df_street_uuids = retrieve_uuid_from_memorix(ctx.obj["env"], ctx.obj['street_uuids'])
    ctx.obj['df_street_uuids'] = df_street_uuids
    ...
    concept_list = turtle_to_list( ctx.obj["concept_turtle"])
    ...
    click.echo("STEP 5: READ ADDED DATASHEET WITH PANDA")
    
    read_data_with_panda( ctx.obj["data"])

    df_alt_streets = read_data_with_panda( ctx.obj["data"])
    ctx.obj['df_alt_streets'] = df_alt_streets
    ...
    record_turtle = process_data(ctx.obj['api'], ctx.obj['df_street_uuids'])
    ...
    click.echo("STEP 7: SOME EXTRA LOGIC")
