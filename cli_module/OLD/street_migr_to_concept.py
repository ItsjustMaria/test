...
# Step 1 function
def retrieve_deed_turtle_from_memorix(api, turtle_description, result):
    ...
    try:
        ...
        errors.append({'fn: retrieve_deed_turtle_from_memorix': [turtle_description, result, response]})

# Step 2 function
def retrieve_concept_turtle_from_memorix(api, deed_turtle, result):
    
    # Get turtle of concept vocabulaire
    response = api.list_concepts( deed_turtle)
    print(response.text,  file=open(result, 'w', encoding='utf-8'))
    return response
...
def retrieve_uuid_from_memorix(env, street_uuids):
    ...
    df_street_uuids = pd.read_csv(street_uuids,
...
def read_data_with_panda(data):
    ...
    df_alt_streets = pd.read_csv(data, 
...
def process_data(api, data):
    ...
    for index, row in data.iterrows():
        log.info(f"START {row[0]}")
        try:
            response = api.get_record(row[0])
            print(response.text)
            if response.status_code != 200:
                ...
                response = api.get_record(row[0])
                if response.status_code != 200:
                    log.error(f"Reading failed for {row[0]}")
                    continue
            ...
            log.error(f"FAILED TRANSFORMATION {row[0]}")
...
def download_record_turtle(ctx):
    ...
    ctx.obj['deed_turtle'] = retrieve_deed_turtle_from_memorix(ctx.obj["api"], ctx.obj['Deed'])
...
def download_concept_vocabulaire(ctx):
    ...
    ctx.obj['concept_turtle'] = retrieve_concept_turtle_from_memorix(ctx.obj["api"], ctx.obj['vocabulair'])
...
def download_record_uuid(ctx):
    ...
    ctx.obj['df_street_uuids'] = retrieve_uuid_from_memorix(ctx.obj["env"], ctx.obj['street_uuids'])
    df_street_uuids = ctx.obj['df_street_uuids']

    click.echo(df_street_uuids)
...
def concept_turtle_to_list(ctx):
    ...
    ctx.obj['concept_list'] = turtle_to_list( ctx.obj["concept_turtle"])
...
def read_added_datasheet(ctx):
    ...
    ctx.obj['df_alt_streets'] = read_data_with_panda( ctx.obj["data"])
...
def concept_turtle_to_excel(ctx):
    ...
    #ctx.obj['df_street_uuids'] = ctx.obj["df_street_uuids"]
...
def pipeline(ctx, data):
    ...
    click.echo("STEP 6: HERE WE START WITH GETTING A RECORD")
