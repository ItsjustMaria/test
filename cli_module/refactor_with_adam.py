...
kwrgs = {
        ...
        'record_uuids' : "data/record_uuids.csv",  
        'test_record_uuids' : "data/test_record_uuids.csv",            #### !!!! Location of deed turtle
        'records' : 'data/records.ttl',      
        'test_records' : 'data/records_with_adam.ttl',      
...
def retrieve_uuid_from_memorix(**kwrgs):
    ...
    response = get_uuid_with_adam.main(env, kwrgs['test_record_uuids']) #Give storage location
    #response = 200  # TEMP

    #Read csv from storage location
    kwrgs['df_record_uuids'] = pd.read_csv(kwrgs['test_record_uuids'], 
        sep=";",             
        dtype={ "uuid": str
           })
    ...
    if os.path.exists(kwrgs['test_records']):     
        # os.remove(records)
        # print(kwrgs['records_deleted_message']) 
        pass

    input(f'UUIDS retrieved at {kwrgs['test_record_uuids']}. Press enter to retrieve turtles.')

    # Get turtles from memorix with help of dataframe uuids  
    for index, row in kwrgs['df_record_uuids'].head(kwrgs['test_amount']).iterrows():
        ...
        if response.status_code != 200:
            ...
            # load the graph
            g = Graph()
            g.parse(kwrgs['test_records'] , format='turtle')
...
def main(**kwrgs):
    ...
    try:
        ...
        kwrgs['predicates'], kwrgs['total_predicates'], kwrgs['result'] = retrieve_uuid_from_memorix(**kwrgs)
        ...
        if kwrgs['result']:  #response.status_code == 200:
            ...
            f"\tEr zijn '{len(kwrgs['predicates'])}' records uit de turtle gehaald," + 
